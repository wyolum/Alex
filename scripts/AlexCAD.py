...
def CornerTwoWay(d1):
    ...
    return parts_db.Part(parts_db.Main, name)

def CornerThreeWay(d1):
    name = f"{d1:.0f}{d1:.0f} Corner Three Way Silver"
    return parts_db.Part(parts_db.Main, name)
...
def parts_db_dialog():
    def select_cb(part):
        ...
        unselect_all()
        scene.append(part, select=True)
        parts_db_dialog.last_lib = part.lib
    parts_db.PartDialog(root, select_cb, parts_db_dialog.last_lib)
parts_db_dialog.last_lib = None
...
# Create view layout using the new modular approach
from packages import view_layout
views, rt_angle_images = view_layout.create_view_layout(
    root,
    sidebar,
    rotate_yaw,
    rotate_pitch,
