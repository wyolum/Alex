...
class SideBar:
    def __init__(self, parent, width, height):
        ...
        self.frame.grid(row=1, column=1, rowspan=10)
        ...
        self.new_part_button = tk.Button(self.frame, command=new_part, text='New ExAl')
...
class CornerTwoWay(ExAl):
    def __init__(self, dim1):
        ExAl.__init__(self, dim1, dim1, dim1)
...
class Interface:
    def __init__(self, part, hotspot, idx):
        self.other_dims = np.ones(3)
        self.other_dims[idx] = 0
        self.engaged = False
    def is_engaged_with(self, thing):
        v = self.orient.T @  (stop - start)
        vhat = v / np.linalg.norm(v)
        
        if np.sum(other_dims @ np.abs(vhat)) < 0.001: ### correct orientation
            if ((np.linalg.norm(start - hotspot) < .001 and vhat[idx] > 0) or
                (np.linalg.norm(stop - hotspot) < .001 and vhat[idx] < 0)):
                out = True
        
class CornerThreeWay(ExAl):
    def __init__(self, dim1):
        if dim1 < 30:
            h = 5
        else:
            h = 6
        ExAl.__init__(self, h, dim1, dim1)

    def fully_engaged(self):
        ...
        out = True
        for interface in self.interfaces:
            if not interface.engaged:
                out = False
                break
        return out
            
    def engaged(self, thing):
        '''
        check all objects to see if an ExAl is in right place and orientation
        '''
        if isinstance(thing, ExAl):
            start,stop = thing.getstartstop()
            out = False
            v = self.orient.T @  (stop - start)
            vhat = v / np.linalg.norm(v)
            hotspots = [
                (self.pos + self.orient @ [0, 0, self.length], 2),
                (self.pos + self.orient @ [0, self.dim1/2, self.dim1/2], 1),
            ]
            n_engaged = 0
            for interface, idx in self.intefaces:
                if interface.is_engaged(thing):
                    out = True
                    break
        else:
            out = False
            for sub in thing:
                out = self.engaged(thing=sub)
                if out:
                    break
        return out

    def render(self, view):
        ExAl.render(self, view)
        s1 = self.dim1 / 2
        s2 = self.dim2 / 2
        l = 2
        if True: #self.engaged(scene):
            color = 'green'
            width = 1 * max([1, np.min([2 * view.get_scale(), 3])])
        else:
            color = 'red'
            width = 1 * np.max([.5, np.min([1 * view.get_scale(), 1.5])])
        start = self.pos + self.orient @ [0, s2, 0]
        stop = self.pos + self.orient @ [0, s2 + l, 0]
        view.create_line(self, start, stop,color, width)


        start = self.pos + self.orient @ [0, -s2, self.length]
        stop = self.pos + self.orient @ [0, -s2, self.length + l]
        view.create_line(self, start, stop, color, width)

        start = self.pos + self.orient @ [-s1, 0, 0]
        stop = self.pos + self.orient @ [-s1 - l,0, 0]
        view.create_line(self, start, stop, color, width)
        start = self.pos + self.orient @ [s1, 0, self.length]
        stop = self.pos + self.orient @ [s1, 0, self.length + l]
        view.create_line(self, start, stop, color, width)
    ...
    def toscad(self):
        ...
        if selected_group.contains(self):
            out[0] = '#' + out[0]
        if False: # not self.engaged(scene):
            out[0] = 'color("orange")' + out[0]
        if abs(angle) > 1e-8:
            vec = q.vec
            vec /= np.linalg.norm(vec)
            out.append(f'  rotate(a={angle / DEG:.0f}, v=[{vec[0]:.4f}, {vec[1]:4f}, {vec[2]:4f}])')
...
class IsoView:
    ...
    def set_scale(self, scale):
        print('1set_scale', scale)
        self.scale = scale
    def get_scale(self):
        return self.scale
    def create_line(self, thing, start, stop, color, width):
        id = self.can.create_line(
            self.offset_x + self.get_scale() * start @ self.x,
            self.offset_y + self.get_scale() * start @ self.y,
            self.offset_x + self.get_scale() * stop @ self.x,
            self.offset_y + self.get_scale() * stop @ self.y,
            tags=(str(thing),),
    ...
    def ondrag(self, event):
        if self.dragging:
            ...
            STEP = 5
            delta = (delta / STEP).astype(int) * STEP
...
tk.Button(root, text="Side").grid(row=4, column=5, sticky="NW")

frontcan = tk.Canvas(root, width = CANVAS_DIM, height=CANVAS_DIM)
frontcan.grid(row=4, column=3);
tk.Button(root, text="Front").grid(row=4, column=3, sticky="NW")
...
top = IsoView(topcan, ihat , -jhat , CANVAS_DIM/2, CANVAS_DIM/2, scale=scale)
side = IsoView(sidecan, jhat , -khat , CANVAS_DIM/2, CANVAS_DIM - 50, scale=scale)
front = IsoView(frontcan, ihat , -khat, CANVAS_DIM/2, CANVAS_DIM - 50, scale=scale)
iso = IsoView(isocan, -iso_x , iso_y , CANVAS_DIM/2, CANVAS_DIM - 50, scale=scale)
...
def cube_dialog(*args):
    ...
    def on_submit(*args):
        ...
        group = Group()
        group.append(ExAl(h - 2 * d1, d1, d2).translate([-l/2 + d1/2, -w/2 + d2/2, d1]))
        group.append(ExAl(h - 2 * d1, d1, d2).translate([-l/2 + d1/2,  w/2 - d2/2, d1]))
        group.append(ExAl(h - 2 * d1, d1, d2).translate([ l/2 - d1/2, -w/2 + d2/2, d1]))
        group.append(ExAl(h - 2 * d1, d1, d2).translate([ l/2 - d1/2,  w/2 - d2/2, d1]))

        if hidden_corners:
            corner = CornerThreeWay(d1)
            part = ExAl(l-2 * corner.length, d1, d2).rotate(pitch=-1).translate([corner.length - l/2, -w/2 + d2/2, d1/2])
        else:
            part = ExAl(l, d1, d2).rotate(pitch=-1).translate([-l/2, -w/2 + d2/2, d1/2])
        group.append(part)
        ...
        part = ExAl(w - 2 * d2, d1, d2).rotate(roll=1).translate([l/2-d1/2, w/2 - d2, d2/2])
        group.append(part)
...
def exal_bom():
    ...
    out = '\n'.join(out)
    if exal_filename:
        base = exal_filename[0]
        if base.endswith('.exal'):
            open(exal_filename[0][:-4] + 'csv', 'w').write(out)
    print(out)
...
def exal_set_titlebar():
    if exal_filename:
        fn = exal_filename[0]
    else:
        fn = ''
    root.winfo_toplevel().title(f"ExAl {fn}")    
def exal_import():
...
filemenu.add_command(label="Open", command=exal_open_dialog)
...
filemenu.add_command(label="Generate BoM", command=exal_bom)
...
partmenu.add_command(label='3-Way Corner (Right)', command=new_3_way_right)
partmenu.add_command(label='3-Way Corner (Left)', command=new_3_way_left)
