### Source: https://code.activestate.com/recipes/580793-tkinter-table-with-scrollbars/
# Author: Miguel Martinez Lopez
# Version: 0.20
### modified by Justin Shaw for Alex CAD
...
class Scrolling_Area(Frame, object):
    ...
    def __init__(self, master, width=None, anchor=N, height=None, mousewheel_speed = 2,
                 scroll_horizontally=True, xscrollbar=None, scroll_vertically=True,
                 yscrollbar=None, outer_background=None, inner_frame=Frame, **kw):
...
def url_shortener(url, maxlen=30):
    if len(url) > maxlen:
        return url[:maxlen-3] +  '...'
    
class Data_Cell(Cell):
    def __init__(self, master, variable, anchor=W, bordercolor=None, borderwidth=1, padx=0, pady=0, 
                 background=None, foreground=None, font=None, wraplength=100):
        ...
        self.message_widget = Label(self, textvariable=variable, font=font, background=background, foreground=foreground,
                                       wraplength=wraplength)
        self.message_widget.pack(expand=True, padx=padx, pady=pady, anchor=anchor)

class Header_Cell(Cell):
    def __init__(self, master, text, bordercolor=None, borderwidth=1, padx=0, pady=0, background=None, 
                 foreground=None, font=None, anchor=CENTER, separator=True):
        Cell.__init__(self, master, background=background, highlightbackground=bordercolor, 
                      highlightcolor=bordercolor, highlightthickness=borderwidth, bd= 0)
...
class Table(Frame):
    def __init__(self, master, columns, column_weights=None, column_minwidths=None, height=500, minwidth=20, minheight=20, 
                 padx=5, pady=5, cell_font=None, cell_foreground="black", cell_background="white", cell_anchor=W, 
                 header_font=None, header_background="white", header_foreground="black", header_anchor=CENTER, 
                 bordercolor = "#999999", innerborder=True, outerborder=True, stripped_rows=("#EEEEEE", "white"), 
                 on_change_data=None, mousewheel_speed = 2, scroll_horizontally=False, scroll_vertically=True,
                 ):
        ...
        for j in range(len(columns)):
            ...
            header_cell = Header_Cell(self._head, text=column_name, borderwidth=self._innerborder_width, font=header_font,
        ...
        self.column_minwidths = column_minwidths
    def _append_n_rows(self, n):
        ...
        for i in range(number_of_rows, number_of_rows+n):
            ...
            for j in range(number_of_columns):
                ...
                wraplength=self.column_minwidths[j]
                cell = Data_Cell(self._body, borderwidth=self._innerborder_width, variable=var, bordercolor=self._bordercolor,
                                 padx=self._padx, pady=self._pady, background=self._stripped_rows[i%len(self._stripped_rows)],
                                 foreground=self._cell_foreground, font=self._cell_font, anchor=self._cell_anchor,
                                 wraplength=wraplength)
                self.cells[-1].append(cell)
                cell.grid(row=i, column=j, sticky=N+E+W+S)
                colname = self._columns[j]
                var.cell = cell
                if colname.strip().lower() == 'url':
                    cell.message_widget.bind('<Button-1>', curry(browseto, cell))
        ...
        for i in range(n):
            for j in range(m):
                ...
                if str(dij).lower().endswith('.stl'):
                    ...
                    img = ''.join(str(self._data_vars[i][0].get()).split())
                    from PIL import Image, ImageTk
                    img = ImageTk.PhotoImage(Image.open(f'STL/2020Alex.png'))
                    self.images.append(img) ### save from garbage collector!
                    #dij = img
                    self._data_vars[i][j].cell.message_widget.config(image=self.images[-1])
                    
                self._data_vars[i][j].set(dij)
...
if __name__ == "__main__":
    ...
    table = Table(root, ["column A", "column B", "column C"], padx=0, pady=0, column_minwidths=[100, 100, 300])
    table.grid(row=0, column=0)
    
    table.set_data(np.arange(99).reshape((-1, 3)))
    table.cell(0,0, " This is a test of the text wrapping system ")
    
    table.insert_row([22,23,24])
    table.insert_row(["25---","26----","this-is-longer-than-two-chars"])
    
    root.update()
    root.geometry("%sx%s"%(root.winfo_reqwidth(),550))
